...
for item in sorted(st.session_state.items, key=lambda x: x["impact"] / max(x["effort"], 1), reverse=True):
    score = round(item["impact"] / max(item["effort"], 1), 1)
    st.write(f"**{item['goal']}** · {item['area']} · Priorità {score}/5 · entro {item['deadline'].strftime('%d/%m/%Y')}")

st.divider()
st.subheader("Aggiungi una decisione")
with st.form("new_item"):
    area = st.selectbox("Area", ["Salute", "Finanze", "Lavoro", "Famiglia", "Apprendimento", "Tempo libero"])
    ...
    if submitted and goal.strip():
        st.session_state.items.append({"area": area, "goal": goal.strip(), "impact": impact, "effort": effort, "deadline": deadline})
